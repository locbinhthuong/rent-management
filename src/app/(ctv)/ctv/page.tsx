...
function DashboardSkeleton() {
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 max-w-6xl mx-auto w-full animate-pulse">
      <div className="bg-white/80 h-[200px] rounded-3xl border border-slate-100 shadow-sm flex items-center justify-center">
        <Loader2 className="w-8 h-8 text-indigo-400 animate-spin" />
      </div>
      <div className="bg-white/80 h-[400px] rounded-3xl border border-slate-100 shadow-sm"></div>
    </div>
  );
}
      ...
      <div className="bg-white/80 backdrop-blur-xl rounded-3xl border border-white shadow-[0_10px_40px_rgba(0,0,0,0.03)] overflow-hidden">
        ...
        {posts.length === 0 ? (
          ...
          <div className="divide-y divide-slate-100/60">
            {posts.map((post) => (
              <div id={`post-${post._id.toString()}`} key={post._id.toString()} className="p-6 flex flex-col xl:flex-row xl:items-center justify-between hover:bg-slate-50/50 transition-colors gap-6">
