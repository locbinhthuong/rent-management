  ...
  return (
      <main className="flex-1 flex flex-col h-screen overflow-y-auto pb-24 md:pb-0">
        <header className="bg-slate-900/50 backdrop-blur-xl p-4 md:p-5 border-b border-white/10 flex justify-between items-center sticky top-0 z-10 shadow-sm">
          <h2 className="text-lg md:text-xl font-bold text-slate-100">Quản lý Cộng tác viên</h2>
        </header>
        ...
        <div className="p-4 md:p-8 pb-20 md:pb-8">
          <div className="bg-slate-900/50 backdrop-blur-xl rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.12)] border border-white/10 overflow-hidden">
            <div className="p-4 md:p-5 border-b border-white/10 flex flex-col md:flex-row md:justify-between md:items-center gap-4 bg-slate-800/50">
              <h3 className="font-bold text-slate-100 flex items-center gap-2">
                <Users className="w-5 h-5 text-cyan-400" /> Danh sách CTV ({ctvs.length})
              </h3>
              <div className="relative w-full md:w-auto">
                <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
                <input type="text" placeholder="Tìm kiếm CTV..." className="pl-9 pr-4 py-2 bg-slate-900 border border-white/20 rounded-lg text-sm text-slate-100 focus:ring-2 focus:ring-cyan-500/50 focus:border-cyan-500/50 outline-none w-full md:w-64 placeholder:text-slate-500" />
              </div>
            </div>
              ...
              <table className="w-full text-left text-sm">
                ...
                <tbody className="divide-y divide-white/10">
                  {ctvs.length === 0 ? (
                    ...
                    ctvs.map((ctv: any) => (
                      <tr key={ctv._id.toString()} className="hover:bg-white/5 transition">
                        <td className="px-6 py-4">
                          <div className="font-bold text-slate-100">{ctv.name}</div>
                          <div className="text-xs text-slate-400 mt-1">ID: {ctv._id.toString().slice(-6)}</div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-slate-100">{ctv.phone || 'Chưa cập nhật'}</div>
                          <div className="text-slate-400 text-xs">{ctv.email}</div>
                        </td>
