  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 max-w-7xl mx-auto w-full animate-pulse">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        {[1, 2, 3, 4].map((i) => (
          <div key={i} className="bg-white/80 p-6 rounded-3xl border border-slate-200 shadow-sm h-36 flex flex-col justify-between">
      ...
      <div className="bg-white/80 h-[400px] rounded-3xl border border-slate-200 shadow-sm flex items-center justify-center">
...
async function AdminDashboardContent() {
  ...
  return (
    <div className="p-4 md:p-8 space-y-6 md:space-y-8 max-w-7xl mx-auto w-full">
      {/* Stats Cards */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <div className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex flex-col gap-4 hover:-translate-y-1 hover:border-cyan-500/30 transition-all duration-300">
        ...
        <div className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex flex-col gap-4 hover:-translate-y-1 hover:border-emerald-500/30 transition-all duration-300">
          <div className="flex items-center justify-between text-slate-600 font-semibold">
          ...
          <div className="text-3xl font-black text-slate-900 tracking-tight">{rentedRoomsCount}/{totalRoomsCount}</div>
          <div className="text-sm text-slate-600 font-medium">Tỷ lệ lấp đầy: <span className="text-emerald-400 font-bold">{occupancyRate}%</span></div>
        </div>
        
        <div className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex flex-col gap-4 hover:-translate-y-1 hover:border-orange-500/30 transition-all duration-300">
          <div className="flex items-center justify-between text-slate-600 font-semibold">
          ...
          <div className="text-3xl font-black text-slate-900 tracking-tight">{pendingPostsCount}</div>
          <div className="text-sm text-orange-400 font-bold">Cần kiểm tra ngay</div>
        </div>
        
        <div className="bg-white/80 backdrop-blur-xl p-6 rounded-3xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex flex-col gap-4 hover:-translate-y-1 hover:border-blue-500/30 transition-all duration-300">
          <div className="flex items-center justify-between text-slate-600 font-semibold">
            <span>Cộng tác viên</span>
            <div className="bg-blue-500/10 p-2.5 rounded-xl border border-blue-500/20"><Users className="w-5 h-5 text-blue-400" /></div>
          </div>
          <div className="text-3xl font-black text-slate-900 tracking-tight">{ctvCount}</div>
          <div className="text-sm text-slate-600 font-medium">Đang hoạt động</div>
        </div>
      </div>
      
      <div className="bg-white/80 backdrop-blur-xl p-6 md:p-8 rounded-3xl border border-slate-200 shadow-[0_8px_30px_rgb(0,0,0,0.12)]">
        <h3 className="text-xl font-extrabold text-slate-900 mb-6">Thống kê dữ liệu</h3>
        {leadStats.length > 0 ? (
          <AdminCharts leadStats={leadStats} />
        ) : (
          <div className="bg-slate-100/80 border border-slate-200 rounded-2xl p-12 text-center flex flex-col items-center justify-center gap-4">
            <FileText className="w-12 h-12 text-slate-600" />
            <p className="text-slate-600 font-medium">Chưa có dữ liệu Khách hàng để hiển thị biểu đồ.</p>
          </div>
        )}
      </div>
    </div>
...
export default async function AdminDashboard() {
  ...
  return (
      <main className="flex-1 flex flex-col h-screen overflow-y-auto relative z-10 pb-24 md:pb-0">
        {/* Header */}
        <header className="bg-white/80 backdrop-blur-md p-4 md:px-8 md:py-6 border-b border-slate-200 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 sticky top-0 z-30">
