...
export default function PostLoading() {
  return (
    <div className="max-w-6xl mx-auto px-4 py-8 mt-16 animate-pulse">
      {/* Breadcrumb Skeleton */}
      <Skeleton className="h-4 w-48 mb-6 bg-slate-200/50" />
      
      {/* Title Skeleton */}
      <Skeleton className="h-10 w-3/4 mb-4 bg-slate-200" />
      <Skeleton className="h-6 w-1/3 mb-6 bg-slate-200/50" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="flex gap-2">
            {[...Array(4)].map((_, i) => (
              <Skeleton key={i} className="w-24 h-24 rounded-lg bg-slate-200/50" />
            ))}
          </div>
        ...
        <div className="space-y-6">
          <Skeleton className="h-64 w-full rounded-2xl bg-slate-200/50" />
