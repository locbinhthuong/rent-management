  ...
  return (
    <div className="w-full flex flex-col h-[calc(100vh-64px)] overflow-hidden bg-slate-50 relative mt-16">
          ...
          <div className="flex flex-col md:flex-row items-center gap-3 overflow-x-auto custom-scrollbar pb-1 md:pb-0">
            ...
            <div className="flex items-center bg-slate-100 rounded-full px-4 py-2 w-full md:w-auto md:min-w-[250px] shrink-0 border border-slate-200 focus-within:border-cyan-400 focus-within:ring-2 focus-within:ring-cyan-100 transition-all">
              ...
              <input 
                ...
                placeholder="Tìm theo d?a di?m, tên..."
            ...
            <div className="flex items-center gap-2 w-full md:w-auto shrink-0 overflow-x-auto pb-1 md:pb-0">
              ...
              <select 
                ...
                className="bg-white border border-slate-200 text-slate-700 text-sm rounded-full px-4 py-2 outline-none hover:border-cyan-400 transition cursor-pointer"
              >
                <option value="">T?nh/Thành</option>
              ...
              <select 
                ...
                disabled={!city}
                className="bg-white border border-slate-200 text-slate-700 text-sm rounded-full px-4 py-2 outline-none hover:border-cyan-400 transition cursor-pointer disabled:opacity-50"
              >
              ...
              <select 
                ...
                className="bg-white border border-slate-200 text-slate-700 text-sm rounded-full px-4 py-2 outline-none hover:border-cyan-400 transition cursor-pointer"
              >
                <option value="">M?c giá</option>
                <option value="0-2000000">Du?i 2 tri?u</option>
                <option value="2000000-5000000">2 - 5 tri?u</option>
                <option value="5000000-10000000">5 - 10 tri?u</option>
                <option value="10000000-999999999">Trên 10 tri?u</option>
              </select>
      ...
      <div className="flex-1 flex w-full h-full overflow-hidden relative">
        ...
        <div className={\\ w-full md:w-[55%] lg:w-[60%] flex-col h-full bg-slate-50 overflow-y-auto custom-scrollbar\}>
          <div className="p-4 md:p-6 lg:px-8">
            <div className="mb-6 flex justify-between items-center">
              <h1 className="text-xl md:text-2xl font-bold font-space tracking-tight text-slate-900">
                Khám phá không gian s?ng
              </h1>
              <span className="text-sm text-slate-500 font-medium">
                {pagination.total} k?t qu?
              </span>
            </div>
            
            {posts.length === 0 ? (
              <div className="text-center py-20 bg-white/80 rounded-2xl border border-slate-200">
                <p className="text-slate-500 font-medium">Không tìm th?y k?t qu? phù h?p. Hãy th? thay d?i b? l?c.</p>
              </div>
            ) : (
        ...
        <div className={\\ w-full md:w-[45%] lg:w-[40%] h-full relative z-10 border-l border-slate-200\}>
      ...
      <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-50">
        ...
        >
          {showMapMobile ? (
            ...
            <><MapIcon className="w-4 h-4" /> B?n d?</>
          )}
        </button>
      </div>
...
;
fs.writeFileSync('src/components/SplitScreenSearch.tsx', content, 'utf8');
